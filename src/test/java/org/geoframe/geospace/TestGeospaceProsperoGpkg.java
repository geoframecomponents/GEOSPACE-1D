...
import org.geoframe.geoet.solvers.JarvisStressFactorSolver;
 ...
 * solver, driven each step by GEOET's root-density/Jarvis-stress/Prospero
 * (transpiration) + Penman-Monteith-FAO (soil evaporation under canopy)
 * solvers and BrokerGEO's two-flux broker. Scenario producing an
 * evaporation/transpiration split rather than one combined ET.
 *
 * @author Concetta D'Amato, Niccolo' Tubini, Michele Bottazzi and Riccardo Rigon
 * @author Andrea Antonello
 */
public class TestGeospaceProsperoGpkg extends GeospaceTestCase {

	public void testProspero() throws Exception {

		String startDate = "2015-04-01 00:00";
		String endDate = "2015-06-01 02:00";

		String inputsPath = getRes("/input/gpkg/CavoneProspero.gpkg");
		String outputPath = getTmpPath("CavoneProsperoOutput", ".gpkg");
		...
		try (ADb inDb = EDb.GEOPACKAGE.getDb()) {
			...
			double etaE = geoetIn.getParameterDouble("jarvisEtaE");
			...
			// ---- GEOET: root density + Jarvis stress factor + Prospero + soil evaporation ----
			...
			inputPreprocessor.latitude = geoetIn.getParameterDouble(GeoetInputsHandler.PARAM_LATITUDE);
			inputPreprocessor.longitude = geoetIn.getParameterDouble(GeoetInputsHandler.PARAM_LONGITUDE);
			...
			JarvisStressFactorSolver jarvis = new JarvisStressFactorSolver();
			...
			jarvis.representativeTranspirationSFModel = "RootDensityWeightedMethod";
			jarvis.representativeEvaporationSFModel = "AverageMethod";
			...
			prospero.input = geoetInput;
			prospero.variables = geoetVars;
			prospero.parameters = new Parameters();
			prospero.leafparameters = leaf;
			prospero.typeOfCanopy = geoetIn.getParameterString(GeoetInputsHandler.PARAM_TYPE_OF_CANOPY);

			PenmanMonteithFAOSoilEvaporationSolverWithCanopy pmSoilEvaporation = new PenmanMonteithFAOSoilEvaporationSolverWithCanopy();
			pmSoilEvaporation.input = geoetInput;
			pmSoilEvaporation.variables = geoetVars;
			pmSoilEvaporation.parameters = new Parameters();

			TotalEvapoTranspirationSolver totalEvapoTranspiration = new TotalEvapoTranspirationSolver();
			totalEvapoTranspiration.input = geoetInput;
			totalEvapoTranspiration.variables = geoetVars;
			totalEvapoTranspiration.parameters = new Parameters();

			// ---- BrokerGEO: splits the coupled transpiration+evaporation across the root/evap zones ----
			...
			brokerInput.etaE = etaE;

			ETsBrokerTwoFluxesSolverMain broker = new ETsBrokerTwoFluxesSolverMain();
			broker.input = brokerInput;
			broker.variables = brokerVars;
			broker.representativeEsModel = FluxSplitMethod.AVERAGE_WATER_WEIGHTED;
			broker.representativeTsModel = FluxSplitMethod.ROOT_WATER_WEIGHTED;
			...
			try (whetgeoIn;
					...
					var humidityIterator = geoetIn.iterateTimeseries(GeoetInputsHandler.VAR_RELATIVE_HUMIDITY, startDate,
							endDate, 500);
					var swDirectIterator = geoetIn.iterateTimeseries(GeoetInputsHandler.VAR_SHORT_WAVE_RADIATION_DIRECT,
							startDate, endDate, 500);
					var swDiffuseIterator = geoetIn.iterateTimeseries(GeoetInputsHandler.VAR_SHORT_WAVE_RADIATION_DIFFUSE,
							startDate, endDate, 500);
					var lwIterator = geoetIn.iterateTimeseries(GeoetInputsHandler.VAR_LONG_WAVE_RADIATION, startDate,
							endDate, 500);
					var laiIterator = geoetIn.iterateTimeseries(GeoetInputsHandler.VAR_LEAF_AREA_INDEX, startDate,
							endDate, 500);
				...
				var pm = new LogProgressMonitor("TestGeospaceProsperoGpkg");
				pm.beginTask(" -> Running GEOSPACE-1D full-stack test", -1);
				int iteration = 0;
				while (topBCIterator.next() && bottomBCIterator.next() && airTempIterator.next()
						&& windIterator.next() && humidityIterator.next() && swDirectIterator.next()
						&& swDiffuseIterator.next() && lwIterator.next() && laiIterator.next()
					...
					inputPreprocessor.inShortWaveRadiationDirect = new HashMap<>(Map.of(0, swDirectIterator.values()));
					inputPreprocessor.inShortWaveRadiationDiffuse = new HashMap<>(Map.of(0, swDiffuseIterator.values()));
					inputPreprocessor.inLongWaveRadiation = new HashMap<>(Map.of(0, lwIterator.values()));
					inputPreprocessor.inLeafAreaIndex = new HashMap<>(Map.of(0, laiIterator.values()));
					...
					prospero.stressSun = jarvis.stressSun;
					prospero.stressShade = jarvis.stressShade;
					prospero.process();

					pmSoilEvaporation.evaporationStressWater = jarvis.evaporationStressWater;
					...
					brokerInput.transpiration = prospero.transpiration;
					brokerInput.evaporation = pmSoilEvaporation.evaporation;
					brokerInput.rootDensity = rootDensitySolver.defRootDensity;
					brokerInput.GnT = jarvis.GnT;
					brokerInput.GnE = jarvis.GnE;
					brokerInput.g = jarvis.g;

					broker.solve();

					stressedETsForNextStep = Arrays.copyOf(broker.StressedETs, KMAX);
					cumulativeET += geoetVars.evapoTranspiration;
					...
					// uptake/ET tables (the split evaporation/transpiration columns get real
					// non-trivial values here, unlike the combined-only PriestleyTaylor/
					// PenmanMonteithFAO scenarios) ----
					...
					geoetOut.evapoTranspiration = geoetVars.evapoTranspiration;
					geoetOut.fluxEvapoTranspiration = geoetVars.fluxEvapoTranspiration;
					geoetOut.evaporation = geoetVars.evaporation;
					geoetOut.fluxEvaporation = geoetVars.fluxEvaporation;
					geoetOut.transpiration = geoetVars.transpiration;
					geoetOut.fluxTranspiration = geoetVars.fluxTranspiration;
					geoetOut.write();
			...
			// output/GEOSPACE/*_waterstress_test02.csv are real, already-committed runs of
			// the OLD test this one replaces (same site, same date range, same
			// lab="waterstress"/lab2="test02" naming).
			assertGpkgColumnMatchesGolden("/golden/TestGeospaceProsperoGpkg/EvapoTranspiration.csv", outputPath,
					"geoframe_geoet_output_results", "timestamp", "evapo_transpiration");
			assertGpkgColumnMatchesGolden("/golden/TestGeospaceProsperoGpkg/FluxEvapoTranspiration.csv", outputPath,
					"geoframe_geoet_output_results", "timestamp", "flux_evapo_transpiration");
			assertGpkgColumnMatchesGolden("/golden/TestGeospaceProsperoGpkg/Transpiration.csv", outputPath,
					"geoframe_geoet_output_results", "timestamp", "transpiration");
			assertGpkgColumnMatchesGolden("/golden/TestGeospaceProsperoGpkg/FluxTranspiration.csv", outputPath,
					"geoframe_geoet_output_results", "timestamp", "flux_transpiration");
			assertGpkgColumnMatchesGolden("/golden/TestGeospaceProsperoGpkg/Evaporation.csv", outputPath,
					"geoframe_geoet_output_results", "timestamp", "evaporation");
			assertGpkgColumnMatchesGolden("/golden/TestGeospaceProsperoGpkg/FluxEvaporation.csv", outputPath,
					"geoframe_geoet_output_results", "timestamp", "flux_evaporation");
		}
	}
}
