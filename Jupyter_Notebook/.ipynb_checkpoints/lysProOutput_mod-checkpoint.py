...
def show_stress(a,d):
    ...
    #kl4 = pd.read_csv(c,skiprows=6,parse_dates=[1])
    ...
    kl2 = pd.read_csv(d,skiprows=6,parse_dates=[1])
...
def compare(a,b):
    ...
    kl2 = kl2.drop(['Format'],axis=1)
    kl2.columns.values[0] = 'Date'
    kl2.columns.values[1] = 'SpikeII_data'
    kl2.SpikeII_data[kl2.SpikeII_data<0]=0
    
    
    fig = px.line()
    fig.add_trace(go.Scatter(x=kl['Date'], y=kl['LysGEO'], mode='lines', name='LysGEO'))
    fig.add_trace(go.Scatter(x=kl['Date'], y=kl2['SpikeII_data'], mode='lines', name='SpikeII_data'))

   #fig.update_xaxes(rangeslider_visible=True)
    
    
    fig.update_layout(
        title='Compare LysGEO and SpikeII data',
        #xaxis_title="Date"
...
def compare2(a,b):
    ...
    fig = px.line()
    fig.add_trace(go.Scatter(x=kl['Date'], y=kl['LysGEO'], mode='lines', name='1'))
    fig.add_trace(go.Scatter(x=kl['Date'], y=kl2['SpikeII_data'], mode='lines', name='2'))
...
def compare3(a,b,c):
    ...
    kl3 = kl3.drop(['Format'],axis=1)
    kl3.columns.values[0] = 'Date'
    kl3.columns.values[1] = 'Benettin_data'
   #kl3.SpikeII_data[kl3.Benettin_data<0]=0
    
    
    fig = px.line()
    fig.add_trace(go.Scatter(x=kl['Date'], y=kl['LysGEO'], mode='lines', name='1'))
    fig.add_trace(go.Scatter(x=kl['Date'], y=kl2['SpikeII_data'], mode='lines', name='2'))
    fig.add_trace(go.Scatter(x=kl['Date'], y=kl3['Benettin_data'], mode='lines', name='3'))
